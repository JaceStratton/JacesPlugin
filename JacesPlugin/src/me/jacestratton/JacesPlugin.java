...
public class JacesPlugin extends JavaPlugin {
    ...
    public boolean tip(Player player, String[] args) {
        if (args.length == 0) {
            ...
            if (exception == null) {
                ...
                if (message == null) {
                    player.sendMessage(ChatColor.RED + "Specify number between 1 and " + tips.getNumberOfTips() + ".");
                    return false;
                }
                if (tipCooldown.hasCooldown(player)) {
                    player.sendMessage(ChatColor.RED + "Guide: " + tipCooldown.getCooldown(player));
    ...
    public boolean addtip(Player player, String[] args) {
        ...
        player.sendMessage(ChatColor.RED + "Can't add an empty tip.");
        return false;
    }
    ...
    public boolean rules(Player player, String[] args) {
        ...
        if (args.length != 0) {
            ...
            try {
                ...
                printError(e);
                player.sendMessage(ChatColor.RED + "Specify page number as digit.");
                return false;
            }
        }
        ...
        if (pageNumber > numberOfPages || pageNumber <= 0) {
            player.sendMessage(ChatColor.RED + "Specify number between 1 and " + numberOfPages);
            return false;
        }
        else {
    ...
    public boolean removeRule(Player player, String[] args) {
        ...
        // Empty list will cause abnormalities.
        if (rules.getRules().size() == 1) {
            player.sendMessage(ChatColor.RED + "Can't remove only rule.");
            return false;
        }
        ...
        if (args.length != 0) {
            ...
            try {
                ...
                printError(e);
                player.sendMessage(ChatColor.RED + "Specify number as digit.");
                return false;
            }
        }
        else {
            player.sendMessage(ChatColor.RED + "Specify rule to remove.");
            return false;
        }

        // If no exception, run second part.
        if (ruleNumber <= 0 || ruleNumber > rules.getRules().size()) {
            player.sendMessage(ChatColor.RED + "Specify number between 1 and " + rules.getRules().size() + ".");
            return false;
        }
